...
for file in $files; do
     python3 load_tweets.py --db=postgresql://postgres:pass@localhost:8888/postgres --inputs="$file"
done


echo 'load denormalized'
for file in $files; do
   zcat $file | sed 's/\\u0000//g' | psql postgresql://postgres:pass@localhost:7777/postgres -c "COPY tweets_jsonb(data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
